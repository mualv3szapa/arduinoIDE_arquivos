...
const int buttonReset = 2;    // Pino de controle do botão
...
void loop() {
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    if (obterDadosDoCompartimento1(compartimento1, numAcionamento1) && 
      ...
      // Aciona os atuadores dos compartimentos com base nos dados obtidos
      acionarAtuador(compartimento1, numAcionamento1);
      delay(500);
      acionarAtuador(compartimento2, numAcionamento2);
      delay(500);
      acionarAtuador(compartimento3, numAcionamento3);
...
void atualizarRelatorio(int medicamentoDiario) {
  if(WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
